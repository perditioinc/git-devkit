"""Tests for git_devkit.badges module."""

import os
import tempfile

from git_devkit.badges import BADGE_END, BADGE_START, BadgeBlock


def test_render_basic():
    bb = BadgeBlock(repo="example/test-repo", workflow="test.yml")
    result = bb.render()
...
def test_render_with_suite():
    bb = BadgeBlock(repo="example/test", suite="Reporium")
    result = bb.render()
...
def test_render_with_metrics():
    bb = BadgeBlock(repo="example/test", metrics={"repos": "826"})
    result = bb.render()
...
def test_update_readme_inserts():
    bb = BadgeBlock(repo="example/test", workflow="ci.yml")
...
def test_update_readme_replaces():
    bb = BadgeBlock(repo="example/test", metrics={"v": "2"})
    with tempfile.NamedTemporaryFile(mode="w", suffix=".md", delete=False) as f:
        f.write(f"# Repo\n\n{BADGE_START}\nold badges\n{BADGE_END}\n\nContent\n")
        path = f.name
